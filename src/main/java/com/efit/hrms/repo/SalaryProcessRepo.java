...
//					Long orgId, String employeeCode, String branchCode, Long month, Long year);
			...
			@Query(nativeQuery = true, value = 
				    ...
				    + "    )\r\n"
				    + "    UNION\r\n"
				    + "    SELECT ss.orgid,ss.employeecode,SS.employeename employee,SS.sumofearning bank,0 cash,0 bankotamount,0 cashotamount,0 bankadvance,0 cashadvance\r\n"
				    + "    FROM salarystructure ss join employee e on ss.employeecode=e.employeecode   \r\n"
				    + "    WHERE STR_TO_DATE(ss.createdon, '%d-%m-%Y %h:%i:%s %p') = (\r\n"
				    + "        SELECT MAX(STR_TO_DATE(s2.createdon, '%d-%m-%Y %h:%i:%s %p')) and e.category=1 \r\n"
				    + "        FROM salarystructure s2\r\n"
				    + "        WHERE s2.employeeCode = ss.employeeCode\r\n"
				    + "          AND s2.orgId = ss.orgId\r\n"
				    + "    )\r\n"
