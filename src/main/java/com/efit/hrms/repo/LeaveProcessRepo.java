...
//	Set<Object[]> getLeaveDetailsforSalaryProcess(Long orgId, Long month, String year, String department, String branch, String type);
	...
	@Query(nativeQuery = true, value = "WITH attendance AS (\r\n"
			...
			+ "			    FROM attendancesummary\r\n"
			+ "			    WHERE orgid=?1 AND finyear=?3 AND (branch=?5 or ?5='ALL') AND month=?2\r\n"
			+ "			),\r\n"
			+ "			ss AS (\r\n"
			+ "			    SELECT ss.*\r\n"
			+ "			    FROM salarystructure ss\r\n"
			...
			+ "			    SELECT employeecode,COALESCE(bank,0)bank,COALESCE(cash,0)cash\r\n"
			+ "			    FROM advanceupload\r\n"
			+ "			    WHERE orgid=?1 AND year=?3 AND (branch=?5 or ?5='ALL') AND month=?2\r\n"
			+ "			),\r\n"
			+ "			j AS (\r\n"
			...
			+ "			LEFT JOIN advance ad ON ad.employeecode = att.employeecode where  e.active=1 and e.type=?6 and (e.department=?4 or ?4='ALL')\r\n"
			+ "			) ,\r\n"
			...
			+ "			t.pf,t.esi,t.pt,t.pf+t.esi+t.bankadvance+t.pt totalDeduction,t.bankadvance,t.banksalary  netpay,(t.totaldays-t.salarydays)lopdays,round((t.fixedsalary/t.totaldays)*(t.totaldays-t.salarydays)) lopamount,t.present,t.sunday,t.otdays,t.otamount  from t where t.employeecode not in(select employeecode from salaryprocess where year=?3 and month=?2 group by employeecode) \r\n"
