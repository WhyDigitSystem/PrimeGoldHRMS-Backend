...
//	Set<Object[]> getCheckInOutReport(Long orgId, String employeeCode, String fromDate, String toDate, String branch);
	...
	@Query(value = "WITH company_cte AS (\r\n"
			...
			+ "      TIMEDIFF(MAX(checkouttime), MIN(checkintime)) AS grosshours,\r\n"
			+ "      SEC_TO_TIME(SUM(seconds_diff)) AS effectivehours,\r\n"
			+ "      SUM(seconds_diff) AS total_seconds\r\n"
			+ "    FROM paired\r\n"
			+ "    GROUP BY empcode, empname, branch, checkindate\r\n"
			+ "),\r\n"
			+ "shift_cte AS (\r\n"
			+ "    SELECT \r\n"
			+ "      sd.employeecode,\r\n"
			...
			+ "    JOIN shiftassign sa \r\n"
			+ "      ON sd.shiftassignid = sa.shiftassignid\r\n"
			+ "    WHERE sa.orgid = ?1\r\n"
			...
			+ "    FROM summary s\r\n"
			+ "    JOIN shift_cte sc\r\n"
			+ "      ON s.empcode = sc.employeecode\r\n"
			+ "     AND s.checkindate BETWEEN sc.effectivefrom AND sc.effectiveto\r\n"
			+ "    JOIN employee e \r\n"
			+ "      ON s.empcode = e.employeecode\r\n"
			+ "    JOIN company_cte c \r\n"
			...
			+ "           SEC_TO_TIME(\r\n"
			+ "             GREATEST(\r\n"
			+ "               (f.total_seconds / 60) - TIMESTAMPDIFF(MINUTE, f.shift_in, f.shift_out),\r\n"
			+ "               0\r\n"
			+ "             ) * 60\r\n"
			+ "           ),\r\n"
			+ "           '%H:%i'\r\n"
